...
USBH_StatusTypeDef  USBH_Process(USBH_HandleTypeDef *phost)
{
  ...
  switch (phost->gState)
  {
    ...
    case HOST_DEV_WAIT_FOR_ATTACHMENT: /* Wait for Port Enabled 等待设备接入 */
    ...
    case HOST_CHECK_CLASS://关键
      ...
      else
      {
        ...
        {
          if (phost->pClass[idx]->ClassCode == phost->device.CfgDesc.Itf_Desc[0].bInterfaceClass)//此处需要修改宏定义进行通过
          {
