...
void Angle_Calcu(short* Accel,short* Gyro)	 
{
	...
	Accel_x = Accel[0]; //x轴加速度值获取
	Accel_y = Accel[1]; //y轴加速度值获取
	Accel_z = Accel[2]; //z轴加速度值获取
	Gyro_x = Gyro[0];  //x轴陀螺仪值获取
	Gyro_y = Gyro[1];  //y轴陀螺仪值获取
	Gyro_z = Gyro[2];  //z轴陀螺仪值获取
	//temperature = 36.53+((double)GetData(TEMP_OUT_H))/340.0;//得到温度值
