...
uint8_t Find_bin_file(char* binPath)
{
    ...
    uint32_t cnt = 0;
    FILINFO fno,FileInf;
    char DiskPath[]="0:",bin_stand[]=".bin",FILE_type[5];
    DIR DirInf;
    uint8_t name_size=0;
    binPath[0]='0';
    binPath[1]=':';
    ...
    for (cnt = 0; ;cnt++)
    {
        ...
        else
        {
            ...
            f_stat(FileInf.fname, &fno);
            FILE_type[4]='\0';
            name_size=strlen((char *)FileInf.fname);
            FILE_type[3]=FileInf.fname[name_size-1];
            FILE_type[2]=FileInf.fname[name_size-2];
            FILE_type[1]=FileInf.fname[name_size-3];
            FILE_type[0]=FileInf.fname[name_size-4];
            if(strcmp(FILE_type,bin_stand)==0)
            {
                strcat(binPath,FileInf.fname);    
//                f_mount(NULL, DiskPath, 0);                
                return 1;
            }
        }
...
uint8_t date_buffer[16*1024];//SD卡数据读取缓存区
/**
  * @brief  将SD卡里的bin文件读取出来，并写入flash
  * @param  bin文件存储路径
  * @retval 是否成功写入bin文件  1:成功  0:失败
  */
uint8_t bin_load(char* bin_PATH)
{
    ...
    float schedule=0;
    file_res=f_open(&binFile,bin_PATH,FA_READ);
    if(file_res!= FR_OK)
    {
        printf("打开文件失败\n");
        return 0;
    ...
    printf("开始执行下载\n");
    BufferSize=f_size(&binFile);
/*-----------------------------------------------------------------------------*/
    //执行flash块擦除
    printf("执行擦除\n");
    num = BufferSize / (64*1024) + 1;
    for(i=0;i<num;i++)
    {
        QSPI_W25Qxx_BlockErase_64K(0+64*1024*i);
        schedule=(float)(i+1)/num;
        printf("执行擦除-----%d%%\n",(int)(schedule*100));
    }
    printf("擦除成功\n");
/*-----------------------------------------------------------------------------*/
    printf("执行下载\n");
    num = BufferSize / (16*1024) + 1;
    printf("%d\n",num);
    for(i=0;i<num;i++)
    {
        f_read(&binFile,date_buffer,16*1024,&read_num);
        QSPI_W25Qxx_WriteBuffer(date_buffer,0+16*1024*i,read_num);
        schedule=((float)(i+1))/num;
        printf("执行下载-----%d%%\n",(int)(schedule*100));
    }
    f_close(&binFile);	  //关闭文件	
    /* 卸载文件系统 */
    f_mount(NULL, "0:", 0);
    printf("下载成功\n");
    return  1;
